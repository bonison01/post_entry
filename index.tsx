...
const loaderPercentage = document.getElementById('loader-percentage') as HTMLParagraphElement;
...
/**
 * Sets the loading state of the application, including progress simulation.
 */
function setLoading(loading: boolean) {
    ...
    if (loading) {
        ...
        }, 200); // Approx. 19 seconds to reach 95%
        ...
        // Jump to 100% and then hide after a short delay
...
/**
 * Escapes a cell's content to be CSV-safe.
 */
function escapeCsvCell(cellData: any): string {
    const stringValue = cellData !== undefined && cellData !== null ? String(cellData) : '';
...
/**
 * Handles downloading the extracted data as a CSV file.
 */
function handleDownloadCsv() {
    ...
    const dataRows = extractedData.map(item => 
        columnOrder.map(key => escapeCsvCell(item[key])).join(',')
    );
